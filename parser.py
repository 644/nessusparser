...
c.execute(f"CREATE TABLE IF NOT EXISTS reportitems ({reportitem_tags})");
c.execute("CREATE TABLE IF NOT EXISTS reports (report_name, reporthost_name, host_ip, netbios_name)")
...
for reportitem in tree.iter('ReportItem'):
	...
	c.execute(f"INSERT INTO reportitems ({cols}) VALUES ({marks})", [e.text for e in elems])
...
for report in tree.iter('Report'):
	...
	for reporthost in report.iter('ReportHost'):
		...
		for hostproperties in reporthost.iter('HostProperties'):
			...
			c.execute("INSERT INTO reports VALUES (?, ?, ?, ?);", (report_name, reporthost_name, host_ip[0], netbios_name[0]))
